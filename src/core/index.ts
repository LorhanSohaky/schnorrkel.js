...
export const _generateHashWithSecret = (publicKeys: Buffer[], hexSecret?: string) => {
  ...
  if (typeof hexSecret === 'string') {
    ...
    if (secretBuffer.length > 33) {
      throw new Error('Secret should be 33 bytes or less')
    }

    internalPublicKeys = [...internalPublicKeys, secretBuffer]
  }
...
const internalMultiSigSign = (nonces: InternalNonces, combinedPublicKey: Buffer, privateKey: Buffer, msgHash: string, publicKeys: ReadonlyArray<Buffer>, publicNonces: ReadonlyArray<InternalPublicNonces>): InternalSignature => {
...
export const _generateL = (publicKeys: ReadonlyArray<Buffer>) => {
  return ethers.utils.keccak256(_concatTypedArrays(publicKeys.slice().sort(Buffer.compare)))
}
...
export const _multiSigSign = (nonces: InternalNonces, combinedPublicKey: Buffer, privateKey: Buffer, msg: string, publicKeys: ReadonlyArray<Buffer>, publicNonces: ReadonlyArray<InternalPublicNonces>, hashFn?: Function | null): InternalSignature => {
...
export const _sumSigs = (signatures: ReadonlyArray<Buffer>): Buffer => {
  const internalSignatures = signatures.slice()

  let combined = bigi.fromBuffer(internalSignatures[0])
  internalSignatures.shift()
  internalSignatures.forEach(sig => {
